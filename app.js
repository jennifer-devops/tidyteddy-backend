...
async function initializePropertyTypePriceData(conn) {
    try {
        const [rows] = await conn.query('SELECT COUNT(*) as count FROM propertyTypePrice');

        if (rows[0].count === 0) {
            const sqlFilePath = path.join(__dirname, 'mysql', 'seeds', 'propertyTypePriceSeed.sql');
            const sqlContent = await fs.readFile(sqlFilePath, 'utf-8');
            await conn.query(sqlContent);
        }
    } catch (error) {
...
async function initializeData(conn, tableName) {
    try {
        ...
        if (result.length === 0) {
            const sqlFilePath = path.join(__dirname, 'mysql', 'seeds', `${tableName}Seed.sql`);
            const sqlContent = await fs.readFile(sqlFilePath, 'utf-8');
            await conn.query(sqlContent);
            await conn.commit();
        }
    ...
    try {
        ...
        for (const { name, file } of tables) {
            ...
            if (existingTables.length === 0) {
                ...
                const seedableTable = ['customers', 'payment', 'discount', 'propertys', 'booking']
